...
test("replays broker flow indicators on a short 7-session window", async ({ page }) => {
  ...
  await expect(page.getByText("2026-05-11", { exact: true })).toHaveCount(0)
  await expect(page.getByRole("switch", { name: "Gunakan roadmap replay" })).toHaveAttribute("aria-checked", "false")
  await firstSnapshotDate.fill("2026-05-11")
  await replayButton.click()
  await expect(page.getByText("2026-05-11", { exact: true })).toBeVisible()
